...
Human-in-the-loop (HITL) is a pattern where an AI agent never acts entirely on its own for consequential work. A person starts the loop — for example, by asking about a project or onboarding issue. The agent investigates and summarizes what it finds. The person reviews that work, can ask for more detail or check links to underlying resources, and must authorize any action before it runs. The agent does the heavy lifting (gathering data, drafting, preparing a resolution); the person provides judgment and final authorization, and can stop the process at any time.

---

## Purpose & Value

Because AI agents speak fluently and may omit small details, the team deploying the agent — not the model vendor — is responsible for its output. HITL addresses that responsibility by:
...
- **Preserving a stop** - A kill switch lets someone halt an agent mid-resolution.
...
- **Third-party exposure** - Potential for false or incomplete output to mislead or harm people outside the immediate team.
- **External-facing output** - Anything posted, published, or sent on the team's behalf.
...
- **Review is always available** - Authorization is a clear, reachable step — not buried or optional for high-risk paths.
- **A qualified reviewer can respond** - Someone able to judge the action is available within a timeframe that does not break the workflow.

---

## When NOT to Use

- **Low-risk plan work** - Avoid mandatory authorization for early gathering, drafting, or proposal work where review adds friction without a clear safety gain.
...
- **Time-critical safe actions** - Skip a required gate for safe, automated responses (for example, auto-scaling or failover) where delay is the larger risk.
...
4. **Modify / override** - Person adjusts the decision or command as needed.
5. **Approve & execute — or kill switch** - Action runs only after approval; otherwise the person stops the agent.

**Shared-platform accountability:** On channels such as Slack or other shared surfaces, the review record should show *who* authorized the action, not only that “someone” approved it. (; )

---

## Recommended components

**Button:** Explicit approve, reject, modify, or kill-switch controls that keep consequential agent work under human authorization.

[Button — PatternFly](https://www.patternfly.org/components/button)

**Modal:** Focused review surface for findings, proposed changes, and authorization before an agent executes.

[Modal — PatternFly](https://www.patternfly.org/components/modal)

**Alert:** Status and risk messaging around pending authorization, failed review, or a halted agent run.

[Alert — PatternFly](https://www.patternfly.org/components/alert)

**Helper text:** Supporting copy that separates recorded facts from inferred claims during review.
